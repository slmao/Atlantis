...
| 0005 | [RenderGraph Foundation (GPU-Independent Graph Core)](0005-render-graph-foundation.md) | `Approved` | Not yet created | Not implemented | Depends on Spec 0003 (RHI/Vulkan windowed foundation) — implemented, dependency satisfied. [ADR-0017](../adr/0017-render-graph-construction-compile-layering.md) and [ADR-0018](../adr/0018-render-graph-dependency-derivation-and-ordering.md) both reached `Accepted` alongside this spec's Human Review Approval, recorded 2026-08-09 (see the spec's own Human Review Approval note — all 16 architectural decisions accepted). This is **not** a joint Spec+Plan Human Review: no Plan exists yet, and this approval does not authorize implementation. The next governance step is drafting a Plan against this Approved spec, per [AGENTS.md](../AGENTS.md). |
...
|---|---|---|---|---|
| 1 | Minimal Renderer | Spec 0005 (RenderGraph Foundation) — `Approved`, no Plan yet, not implemented | Minimal mesh/depth/camera/material closed loop, built only on Core, RHI, and RenderGraph | Candidate — spec not yet drafted |
