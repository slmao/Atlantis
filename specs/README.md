...
|---|---|---|---|---|
| 1 | Minimal Renderer | Spec 0005 (RenderGraph Foundation) — `Approved`, implemented; Spec 0006 (RHI / RenderGraph Frame Execution Foundation) — `Approved`, implemented | Minimal mesh/depth/camera/material closed loop, built only on Core, RHI, and RenderGraph | Candidate — spec not yet drafted (both dependencies now satisfied; still requires its own Spec/Plan/ADR before implementation) |
