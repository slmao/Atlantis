...
| 0005 | [RenderGraph Foundation (GPU-Independent Graph Core)](0005-render-graph-foundation.md) | `In Review` | Not yet created | Not implemented | Depends on Spec 0003 (RHI/Vulkan windowed foundation) — implemented, dependency satisfied. Requires [ADR-0017](../adr/0017-render-graph-construction-compile-layering.md) and [ADR-0018](../adr/0018-render-graph-dependency-derivation-and-ordering.md) (both `Proposed`) to reach `Accepted` before this spec can move to `Approved`. `In Review` reflects that the spec's own internal consistency review is complete, not that a human has reviewed or approved it — sixteen architectural decisions remain pending Human Review (see the spec's own Risks & Open Questions). This status does not authorize a Plan or implementation. |
...
|---|---|---|---|---|
| 1 | Minimal Renderer | Spec 0005 (RenderGraph Foundation) — `In Review`, not implemented | Minimal mesh/depth/camera/material closed loop, built only on Core, RHI, and RenderGraph | Candidate — spec not yet drafted |
