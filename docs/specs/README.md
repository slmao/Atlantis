...
| 0042 | [Transparency](0042-transparency.md) | Approved (2026-09-23, chat confirmation; reviewed in this Spec's own branch PR) | None yet — Plan drafting authorized | Not started | Spec 0036 workflow ④; alpha test (`MASK`) and alpha blend (`BLEND`) on the four PBR kinds, schema 7 → 8; blended draws sorted back-to-front inside `drawFrame()` and never write depth. [ADR-0090](../adr/0090-transparency-blend-state-draw-order-and-depth-write.md) (`Accepted`) — RHI `ColorBlendMode`, draw order, depth-write policy. Bistro's transmission glass deferred to ⑦ (ruling O3). |
