...
// Section P6/ADR-0074 Section 1, and Plan 0035 Milestone 2/ADR-0081: the
// runtime material artifact's binary layout -- a fixed 72-byte record
...
// ADR-0081): magic(0,8) schema_version(8,4) kind(12,4) texture_asset(16,8)
// filter(24,4) address_mode(28,4) base_color_factor(32,16)
// metallic_factor(48,4) roughness_factor(52,4)
// normal_map_texture_asset_id(56,8) clearcoat_factor(64,4)
// clearcoat_roughness(68,4) -- total 72, a provable sum (64 existing +
// 8), never a struct with implicit padding.
//
...
// payload -- the entire record is a fixed 72 bytes for schema version 4,
// so decodeMaterialArtifact() rejects any size other than exactly 72
// bytes (UnexpectedSize), not merely "too small" (TruncatedHeader) --
// including a real, old, 64-byte schema-version-3 (or 56-byte
// schema-version-2, or 32-byte schema-version-1) artifact, all rejected
// outright (no dual-version reader).

// Plan 0035 Milestone 2 (ADR-0081): schema bumped 3 -> 4, header size
// 64 -> 72 (a real, deliberate deviation from ADR-0081's own "one bump
// per BRDF slice" plan -- clearcoatFactor/clearcoatRoughness are two
// new float fields, 8 bytes, appended at the existing tail, offset
// 64-71). Same rationale as every prior bump (f68234b, b9fe39d): no
// dual-version reader, every existing artifact at the old version/size
// is rejected outright, not silently accepted.
inline constexpr std::uint32_t kMaterialArtifactSchemaVersion = 4;
inline constexpr std::size_t kMaterialArtifactHeaderSizeBytes = 72;

// Every MaterialKind uses this identical 72-byte layout -- never a
// per-kind-length record (ADR-0066 item 3). baseColorFactor/
// metallicFactor/roughnessFactor/clearcoatFactor/clearcoatRoughness are
...
// PbrClearcoat's reads the last two. normalMapTexture (`0` = none) is
// legal (non-zero) only for PbrDirectLit or PbrClearcoat -- enforced at
// parse time, not here.
...
// are the caller's own already-validated values (checked by
// cookMaterial() before calling this) -- this function is a pure,
