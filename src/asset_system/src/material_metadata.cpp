...
constexpr std::string_view kNormalMapTexturePrefix = "normal_map_texture: ";
// Plan 0035 Milestone 2/ADR-0081: two more, unconditionally present
// fields, same "no optional field" discipline as normal_map_texture
// above.
...
// Plan 0035 Milestone 3/ADR-0081: two more, unconditionally present
// fields, same "no optional field" discipline as clearcoat_factor/
...
// absent), matching this sidecar's own existing "no optional field"
// discipline (it is machine-generated by cookMaterial(), never
// hand-authored, unlike the authoring source grammar). Plan 0035
...
// mirrors the kKindLitTextured widening above exactly.
...
atlantis::Result<MaterialMetadata, MetadataParseError> parseMaterialMetadata(std::string_view text) {
  ...
  for (std::size_t i = 0; i < 3; ++i) {
    if (!parseFloatToken(sheenColorTokens[i], metadata.sheenColor[i])) {
      return ResultT::Err(MetadataParseError::MalformedValue);
    }
  }
...
std::string serializeMaterialMetadata(const MaterialMetadata& metadata) {
  ...
  } else if (metadata.kind == MaterialKind::PbrDirectLit) {
  ...
  out += '\n';
  out += kNormalMapTexturePrefix;
  out += toHexString(metadata.normalMapTexture);
  out += '\n';
  out += kClearcoatFactorPrefix;
  out += formatFloat(metadata.clearcoatFactor);
  out += '\n';
  out += kClearcoatRoughnessPrefix;
  out += formatFloat(metadata.clearcoatRoughness);
  out += '\n';
  out += kSheenColorPrefix;
  for (std::size_t i = 0; i < 3; ++i) {
    if (i != 0) out += ' ';
    out += formatFloat(metadata.sheenColor[i]);
  }
  out += '\n';
