...
TEST_CASE("parseMaterialSource rejects an unknown source version", "[asset_system][material]") {
  // Plan 0035 Milestone 4/ADR-0081: this literal must name a version
  // still genuinely unrecognized now that 6 (this round's own bump) is
  // the real, accepted version -- 7 here, not 6.
  const auto result = parseMaterialSource(
