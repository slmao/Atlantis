...
TEST_CASE("A camera fog group survives the complete source->cook->artifact->decode->World chain (Plan 0043)",
          "[world][scene][fog]") {
  // Plan 0043 P3: the exposure test's shape above, for the five fog
  ...
  const auto camera = world.getCamera(*world.activeCamera());
  REQUIRE(camera.isOk());
  CHECK(camera.value().exposureCompensationEv == 0.5f);
  const atlantis::world::CameraFog& fog = camera.value().fog;
  CHECK(fog.density == 0.05f);
  CHECK(fog.height == -1.5f);
  CHECK(fog.heightFalloff == 0.25f);
  CHECK(fog.maxOpacity == 0.9f);
  CHECK(fog.color.x == 0.5f);
  CHECK(fog.color.y == 0.75f);
  CHECK(fog.color.z == 3.0f);
}

TEST_CASE("A camera with no fog group instantiates with fog off (Plan 0043)", "[world][scene][fog]") {
  const ValidatedSceneData scene = cookAndDecodeScene(kThreeNodeSceneSource);
  World world = fromValidatedSceneData(scene);
  REQUIRE(world.activeCamera().has_value());
  const auto camera = world.getCamera(*world.activeCamera());
  REQUIRE(camera.isOk());
  const atlantis::world::CameraFog defaults;
  CHECK(camera.value().fog.density == 0.0f);
  CHECK(camera.value().fog.height == defaults.height);
