...
TEST_CASE("Height fog (Plan 0043 P9): pbr_direct_lit fogs its surfaces, and every other pixel stays byte-identical",
          "[image_regression][gpu][fog]") {
  auto fixtureResult = atlantis::image_regression::setUpPbrMaterialDemoFixture(buildTestConfig());
  REQUIRE(fixtureResult.isOk());
  auto& fixture = fixtureResult.value();
  atlantis::image_regression::checkSaturatedFogDifferential(
      fixture, [](auto& f) { return atlantis::image_regression::renderPbrMaterialDemoFrame(f); }, "materials/pbr_dielectric_rough.material.txt");
  REQUIRE(fixture.device->waitIdle().isOk());
}
