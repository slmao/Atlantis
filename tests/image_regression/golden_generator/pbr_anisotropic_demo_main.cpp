...
// Plan 0035 Milestone 4 (ADR-0042): standalone developer tool -- never
// CTest-registered, mirrors pbr_sheen_demo_main.cpp's own
// #ifdef-branching two-golden-generator-executables-share-one-main.cpp
// structure exactly, branching here between the IBL-only 3-sphere
// rotation sweep (pbr_anisotropic_demo) and the normal-mapped 1-sphere
// variant (pbr_anisotropic_normal_map_demo).
...
#ifdef ATLANTIS_ANISOTROPIC_NORMAL_MAP_GOLDEN_GENERATOR
inline constexpr const char* kGeneratorName =
    "atlantis_image_regression_pbr_anisotropic_normal_map_demo_golden_generator";
inline constexpr const char* kGeneratorExample =
    "pbr_anisotropic_normal_map_demo/pbr_anisotropic_normal_map_demo_512x512_rgba8unorm";
#else
inline constexpr const char* kGeneratorName = "atlantis_image_regression_pbr_anisotropic_demo_golden_generator";
inline constexpr const char* kGeneratorExample = "pbr_anisotropic_demo/pbr_anisotropic_demo_512x512_rgba8unorm";
#endif
...
// Mirrors pbr_sheen_demo_main.cpp's own identical population of every
// built-in shader pair from configure-time compile definitions, plus
// the two anisotropic shader trios (both branches need both trios --
// realizePendingMaterials() always takes them, whether or not this
// particular scene realizes a normal-mapped anisotropic material).
...
int main(int argc, char** argv) {
  ...
  auto fixtureResult = atlantis::image_regression::setUpPbrAnisotropicDemoFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpPbrAnisotropicDemoFixture() failed");
    return 1;
  }
  atlantis::image_regression::PbrAnisotropicDemoFixture fixture = std::move(fixtureResult.value());

  auto renderResult = atlantis::image_regression::renderPbrAnisotropicDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderPbrAnisotropicDemoFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = atlantis::image_regression::kPbrAnisotropicDemoExtentPixels;
  provenance.extentHeight = atlantis::image_regression::kPbrAnisotropicDemoExtentPixels;
  provenance.format = "Rgba8Unorm";
  provenance.environmentSourceSha256 = ATLANTIS_PBR_ANISOTROPIC_DEMO_ENVIRONMENT_SOURCE_SHA256;
  provenance.environmentArtifactSha256 = ATLANTIS_PBR_ANISOTROPIC_DEMO_ENVIRONMENT_ARTIFACT_SHA256;
  provenance.environmentCookerSettings = "schema=1,face=256,mips=9,dfg=128,samples=1024";
  // ADR-0042 "Initial baseline" category: no prior golden exists for
  // either anisotropic scene -- this is each one's first-ever capture.
#ifdef ATLANTIS_ANISOTROPIC_NORMAL_MAP_GOLDEN_GENERATOR
  provenance.goldenUpdateReason =
      "Initial baseline: Plan 0035 Milestone 4 anisotropic materials, normal-mapped IBL variant";
#else
  provenance.goldenUpdateReason =
      "Initial baseline: Plan 0035 Milestone 4 anisotropic materials, IBL-only rotation sweep";
#endif
