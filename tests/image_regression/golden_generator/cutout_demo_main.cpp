...
// Plan 0042 Milestone 2: standalone, never CTest-registered golden
// generator for the alpha-test cutout scene -- the HDR roll-off
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_cutout_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_cutout_demo_golden_generator "
        "cutout_demo/cutout_demo_512x512_rgba8unorm");
    return 2;
  }
  ...
  auto fixtureResult = setUpCutoutDemoFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpCutoutDemoFixture() failed");
    return 1;
  }
  CutoutDemoFixture fixture = std::move(fixtureResult.value());

  auto renderResult = renderCutoutDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderCutoutDemoFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = atlantis::image_regression::kCutoutDemoExtentPixels;
  provenance.extentHeight = atlantis::image_regression::kCutoutDemoExtentPixels;
