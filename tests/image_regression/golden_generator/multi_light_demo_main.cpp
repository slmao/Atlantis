...
// Plan 0040 Milestone 3: the multi_light_demo golden generator -- a copy
// of lighting_demo_main.cpp with only the scene paths and the golden
// name changed (multi_light_demo reuses LightingDemoFixture unchanged).
...
// Mirrors lighting_demo_main.cpp's own identical population of the
// multi_light_demo_scene's own cooked paths and both built-in shader
...
[[nodiscard]] BootstrapConfig buildConfig() {
  ...
  // too, even though multi_light_demo_scene never realizes one.
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_multi_light_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_multi_light_demo_golden_generator "
        "multi_light_demo/multi_light_demo_512x512_rgba8unorm");
    return 2;
  }
