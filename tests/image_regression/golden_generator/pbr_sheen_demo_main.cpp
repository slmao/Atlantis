...
// Plan 0035 Milestone 3 (ADR-0042): standalone developer tool -- never
// CTest-registered, mirrors pbr_clearcoat_demo_main.cpp's own
...
// (pbr_sheen_demo) and the normal-mapped 1-sphere variant
// (pbr_sheen_normal_map_demo).
...
#ifdef ATLANTIS_SHEEN_NORMAL_MAP_GOLDEN_GENERATOR
inline constexpr const char* kGeneratorName = "atlantis_image_regression_pbr_sheen_normal_map_demo_golden_generator";
inline constexpr const char* kGeneratorExample =
    "pbr_sheen_normal_map_demo/pbr_sheen_normal_map_demo_512x512_rgba8unorm";
#else
inline constexpr const char* kGeneratorName = "atlantis_image_regression_pbr_sheen_demo_golden_generator";
inline constexpr const char* kGeneratorExample = "pbr_sheen_demo/pbr_sheen_demo_512x512_rgba8unorm";
#endif
...
// Mirrors pbr_clearcoat_demo_main.cpp's own identical population of
// every built-in shader pair from configure-time compile definitions,
// plus the two sheen shader trios (both branches need both trios --
// realizePendingMaterials() always takes them, whether or not this
// particular scene realizes a normal-mapped sheen material).
...
int main(int argc, char** argv) {
  ...
  auto fixtureResult = atlantis::image_regression::setUpPbrSheenDemoFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpPbrSheenDemoFixture() failed");
    return 1;
  }
  atlantis::image_regression::PbrSheenDemoFixture fixture = std::move(fixtureResult.value());

  auto renderResult = atlantis::image_regression::renderPbrSheenDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderPbrSheenDemoFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = atlantis::image_regression::kPbrSheenDemoExtentPixels;
  provenance.extentHeight = atlantis::image_regression::kPbrSheenDemoExtentPixels;
  provenance.format = "Rgba8Unorm";
  provenance.environmentSourceSha256 = ATLANTIS_PBR_SHEEN_DEMO_ENVIRONMENT_SOURCE_SHA256;
  provenance.environmentArtifactSha256 = ATLANTIS_PBR_SHEEN_DEMO_ENVIRONMENT_ARTIFACT_SHA256;
  ...
  // either sheen scene -- this is each one's first-ever capture.
#ifdef ATLANTIS_SHEEN_NORMAL_MAP_GOLDEN_GENERATOR
  provenance.goldenUpdateReason = "Initial baseline: Plan 0035 Milestone 3 sheen materials, normal-mapped IBL variant";
#else
  provenance.goldenUpdateReason =
      "Initial baseline: Plan 0035 Milestone 3 sheen materials, IBL-only sheen-roughness sweep";
#endif
