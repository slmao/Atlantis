...
// Plan 0035 Milestone 5 (ADR-0042): standalone developer tool -- never
// CTest-registered, mirrors pbr_anisotropic_demo_main.cpp's own overall
// shape, minus the #ifdef branching (this scene has only one variant,
// not a normal-map pair).
...
inline constexpr const char* kGeneratorName = "atlantis_image_regression_pbr_materials_showcase_golden_generator";
inline constexpr const char* kGeneratorExample = "pbr_materials_showcase/pbr_materials_showcase_768x768_rgba8unorm";
...
// Mirrors pbr_anisotropic_demo_main.cpp's own identical population of
// every built-in shader pair from configure-time compile definitions,
// plus the three real showcase shader trios (clearcoat/sheen/
...
int main(int argc, char** argv) {
  ...
  auto fixtureResult = atlantis::image_regression::setUpPbrMaterialsShowcaseFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpPbrMaterialsShowcaseFixture() failed");
    return 1;
  }
  atlantis::image_regression::PbrMaterialsShowcaseFixture fixture = std::move(fixtureResult.value());

  auto renderResult = atlantis::image_regression::renderPbrMaterialsShowcaseFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderPbrMaterialsShowcaseFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = atlantis::image_regression::kPbrMaterialsShowcaseExtentPixels;
  provenance.extentHeight = atlantis::image_regression::kPbrMaterialsShowcaseExtentPixels;
  provenance.format = "Rgba8Unorm";
  provenance.environmentSourceSha256 = ATLANTIS_PBR_MATERIALS_SHOWCASE_ENVIRONMENT_SOURCE_SHA256;
  provenance.environmentArtifactSha256 = ATLANTIS_PBR_MATERIALS_SHOWCASE_ENVIRONMENT_ARTIFACT_SHA256;
  ...
  // this scene -- this is its first-ever capture.
  provenance.goldenUpdateReason =
      "Initial baseline: Plan 0035 Milestone 5 showcase scene, ~28-sphere fan/arc under warehouse_interior IBL";
