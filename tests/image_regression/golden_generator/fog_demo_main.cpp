...
// Plan 0043 Milestone 2: standalone, never CTest-registered golden
// generator for the three height-fog goldens -- the cutout generator's
// ADR-0042 clean-commit/provenance/capture flow unchanged. One executable
// serves all three, choosing the scene and fixture alias from the golden
// name's directory (fog_distance_demo/, fog_height_demo/, fog_dark_demo/):
// the lit two via PbrMaterialDemoFixture, the dark one via
// PbrNormalMapDemoFixture. The fog is each scene's own.
...
void printProvenanceFieldIfDifferent(const char* fieldName, const std::string& oldValue,
  ...
  if (oldValue != newValue) ATLANTIS_LOG_INFO("  {}: {} -> {}", fieldName, oldValue, newValue);
}
...
// Renders the named golden's scene once, fog as authored; std::nullopt on
// an unknown name or a failed setup/render/waitIdle.
[[nodiscard]] std::optional<PixelBuffer> renderFogGolden(const std::string& goldenName) {
  const auto startsWith = [&goldenName](const char* prefix) { return goldenName.rfind(prefix, 0) == 0; };
  if (startsWith("fog_distance_demo/") || startsWith("fog_height_demo/")) {
    const SceneFiles& scene = startsWith("fog_distance_demo/") ? kFogDistanceScene : kFogHeightScene;
    auto fixtureResult = atlantis::image_regression::setUpFogLitDemoFixture(buildLitConfig(scene));
    if (fixtureResult.isErr()) return std::nullopt;
    auto& fixture = fixtureResult.value();
    auto renderResult = atlantis::image_regression::renderFogLitDemoFrame(fixture);
    if (fixture.device->waitIdle().isErr() || renderResult.isErr()) return std::nullopt;
    return std::move(renderResult.value());
  }
  if (startsWith("fog_dark_demo/")) {
    auto fixtureResult = atlantis::image_regression::setUpFogDarkDemoFixture(
        buildDarkConfig(), ATLANTIS_pbr_normal_mapped_control_ARTIFACT_PATH,
        ATLANTIS_pbr_normal_mapped_control_METADATA_PATH);
    if (fixtureResult.isErr()) return std::nullopt;
    auto& fixture = fixtureResult.value();
    auto renderResult = atlantis::image_regression::renderFogDarkDemoFrame(fixture);
    if (fixture.device->waitIdle().isErr() || renderResult.isErr()) return std::nullopt;
    return std::move(renderResult.value());
  }
  return std::nullopt;
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_fog_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_fog_demo_golden_generator "
        "fog_distance_demo/fog_distance_demo_512x512_rgba8unorm");
    return 2;
  }
  ...
  std::optional<PixelBuffer> rendered = renderFogGolden(goldenName);
  if (!rendered.has_value()) {
    ATLANTIS_LOG_ERROR("rendering {} failed (unknown golden name, or setup/render/waitIdle failed)", goldenName);
    return 1;
  }
  const PixelBuffer capturedPixels = std::move(*rendered);
  ...
  provenance.extentWidth = capturedPixels.width;
  provenance.extentHeight = capturedPixels.height;
  ...
  sidecarOut << serializeGoldenProvenance(provenance);
  if (!sidecarOut) {
