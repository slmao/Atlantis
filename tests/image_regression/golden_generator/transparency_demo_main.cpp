...
// Plan 0042 Milestone 3: standalone, never CTest-registered golden
// generator for the alpha-blend transparency scene -- the HDR roll-off
// generator's ADR-0042 clean-commit/provenance/capture flow unchanged, over
// PbrMaterialDemoFixture reused by alias.
...
[[nodiscard]] BootstrapConfig buildConfig() {
  ...
  config.sceneArtifactPath = ATLANTIS_transparency_demo_scene_ARTIFACT_PATH;
  config.sceneMetadataPath = ATLANTIS_transparency_demo_scene_METADATA_PATH;
  config.sceneDependencyManifestPath = ATLANTIS_transparency_demo_scene_MANIFEST_PATH;
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_transparency_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_transparency_demo_golden_generator "
        "transparency_demo/transparency_demo_512x512_rgba8unorm");
    return 2;
  }
  ...
  auto fixtureResult = setUpTransparencyDemoFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpTransparencyDemoFixture() failed");
    return 1;
  }
  TransparencyDemoFixture fixture = std::move(fixtureResult.value());

  auto renderResult = renderTransparencyDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderTransparencyDemoFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = atlantis::image_regression::kTransparencyDemoExtentPixels;
  provenance.extentHeight = atlantis::image_regression::kTransparencyDemoExtentPixels;
