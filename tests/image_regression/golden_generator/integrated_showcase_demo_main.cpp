...
// pbr_material_demo_main.cpp's own exact structure (Plan 0028
// Milestone 4). ADR-0042's own two-phase golden capture process:
// Milestone 4 only ever runs this tool to produce a *candidate*
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_integrated_showcase_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_integrated_showcase_demo_golden_generator "
  ...
  auto fixtureResult = setUpIntegratedShowcaseDemoFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpIntegratedShowcaseDemoFixture() failed");
    return 1;
  }
  IntegratedShowcaseDemoFixture fixture = std::move(fixtureResult.value());

  // Plan 0028's own default, real-caster path -- the only one this
  // generator ever uses (Spec 0028 FR9: the committed golden always
  // reflects a real shadow, never the R2 no-caster comparison variant).
  auto renderResult = renderIntegratedShowcaseDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderIntegratedShowcaseDemoFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = kIntegratedShowcaseDemoExtentPixels;
  provenance.extentHeight = kIntegratedShowcaseDemoExtentPixels;
  provenance.format = "Rgba8Unorm";
  provenance.goldenUpdateReason = "new golden, Spec 0028 integrated multi-object showcase scene";
