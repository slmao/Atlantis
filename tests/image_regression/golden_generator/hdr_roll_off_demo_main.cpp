...
// Plan 0024 Milestone 10: standalone, never CTest-registered golden
// generator for the focused HDR roll-off scene. It keeps ADR-0042's
// existing clean-commit/provenance/capture flow identical to every
// earlier generator; only the fixture invoked below is new.
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_hdr_roll_off_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_hdr_roll_off_demo_golden_generator "
  ...
  auto fixtureResult = setUpHdrRollOffDemoFixture(buildConfig());
  if (fixtureResult.isErr()) {
    ATLANTIS_LOG_ERROR("setUpHdrRollOffDemoFixture() failed");
    return 1;
  }
  HdrRollOffDemoFixture fixture = std::move(fixtureResult.value());

  auto renderResult = renderHdrRollOffDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderHdrRollOffDemoFrame() failed");
    return 1;
  }
  ...
  provenance.extentWidth = atlantis::image_regression::kHdrRollOffDemoExtentPixels;
  provenance.extentHeight = atlantis::image_regression::kHdrRollOffDemoExtentPixels;
