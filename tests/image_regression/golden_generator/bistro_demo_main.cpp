...
// bistro_demo golden generator -- the assembled Bistro scene (the build
// step's own cooked set and overlay) through PbrNormalMapDemoFixture, no
// environment (ruling O1), the overlay camera's fog and bloom. Built only
// when the Bistro build step exists (content present at configure time).
// Beside ADR-0042's provenance it records the content pin (sidecar schema
// 3): the fetch script's last commit and its LF-normalized SHA-256.
...
[[nodiscard]] BootstrapConfig buildConfig() {
  ...
  config.bloomDownsampleVertexShaderSpirvPath =
      std::string(ATLANTIS_BLOOM_DOWNSAMPLE_SHADER_DIR) + "/bloom_downsample.vert.spv";
  config.bloomDownsampleVertexShaderReflectionPath =
      std::string(ATLANTIS_BLOOM_DOWNSAMPLE_SHADER_DIR) + "/bloom_downsample.vert.refl.json";
  config.bloomDownsampleFragmentShaderSpirvPath =
      std::string(ATLANTIS_BLOOM_DOWNSAMPLE_SHADER_DIR) + "/bloom_downsample.frag.spv";
  config.bloomDownsampleFragmentShaderReflectionPath =
      std::string(ATLANTIS_BLOOM_DOWNSAMPLE_SHADER_DIR) + "/bloom_downsample.frag.refl.json";
  config.bloomUpsampleVertexShaderSpirvPath =
      std::string(ATLANTIS_BLOOM_UPSAMPLE_SHADER_DIR) + "/bloom_upsample.vert.spv";
  config.bloomUpsampleVertexShaderReflectionPath =
      std::string(ATLANTIS_BLOOM_UPSAMPLE_SHADER_DIR) + "/bloom_upsample.vert.refl.json";
  config.bloomUpsampleFragmentShaderSpirvPath =
      std::string(ATLANTIS_BLOOM_UPSAMPLE_SHADER_DIR) + "/bloom_upsample.frag.spv";
  config.bloomUpsampleFragmentShaderReflectionPath =
      std::string(ATLANTIS_BLOOM_UPSAMPLE_SHADER_DIR) + "/bloom_upsample.frag.refl.json";
  config.bloomCompositeVertexShaderSpirvPath =
      std::string(ATLANTIS_BLOOM_COMPOSITE_SHADER_DIR) + "/bloom_composite.vert.spv";
  config.bloomCompositeVertexShaderReflectionPath =
      std::string(ATLANTIS_BLOOM_COMPOSITE_SHADER_DIR) + "/bloom_composite.vert.refl.json";
  config.bloomCompositeFragmentShaderSpirvPath =
      std::string(ATLANTIS_BLOOM_COMPOSITE_SHADER_DIR) + "/bloom_composite.frag.spv";
  config.bloomCompositeFragmentShaderReflectionPath =
      std::string(ATLANTIS_BLOOM_COMPOSITE_SHADER_DIR) + "/bloom_composite.frag.refl.json";
  ...
  // Ruling O1: no environment -- the night street is lit by its own
  // lights (the fixture treats empty environment paths as "none").
...
int main(int argc, char** argv) {
  ...
  if (argc != 2) {
    ATLANTIS_LOG_ERROR("usage: atlantis_image_regression_bistro_demo_golden_generator <golden-name>");
    ATLANTIS_LOG_ERROR(
        "  e.g.: atlantis_image_regression_bistro_demo_golden_generator "
        "bistro_demo/bistro_demo_512x512_rgba8unorm");
    return 2;
  }
  const std::string goldenName = argv[1];
  if (goldenName.rfind("bistro_demo/", 0) != 0) {
  ...
  // ADR-0095 Decision 2: the content pin -- the fetch script's last commit
  // (its SHA-256 pins every fetched file) and the script's own SHA-256.
  const auto pinCommitResult =
      runGitCommand("git log -1 --format=%H -- \"" + std::string(ATLANTIS_BISTRO_FETCH_SCRIPT_PATH) + "\"");
  if (!pinCommitResult.has_value() || pinCommitResult->exitCode != 0 || trim(pinCommitResult->stdOut).empty()) {
    ATLANTIS_LOG_ERROR("git log for the fetch script failed -- it must be committed");
    return 1;
  }
  const std::string contentSourceCommit = trim(pinCommitResult->stdOut);

  const std::string environmentFilePath = ATLANTIS_IMAGE_REGRESSION_ENVIRONMENT_FILE;
  const auto trackedCheckResult = runGitCommand("git ls-files --error-unmatch \"" + environmentFilePath + "\" 2>nul");
  ...
  // Real shadow casters and no explicit BloomInput -- the overlay
  // camera's bloom= group drives the chain (Plan 0044 P10).
  auto renderResult = renderPbrNormalMapDemoFrame(fixture);
  const auto finalWaitResult = fixture.device->waitIdle();
  if (renderResult.isErr()) {
    ATLANTIS_LOG_ERROR("renderPbrNormalMapDemoFrame() failed");
    return 1;
  }
  ...
  provenance.contentSourceCommit = contentSourceCommit;
  provenance.contentFetchScriptSha256 = ATLANTIS_BISTRO_FETCH_SCRIPT_SHA256;
  ...
  ATLANTIS_LOG_INFO("  content_source_commit: {}", provenance.contentSourceCommit);
  return 0;
}
