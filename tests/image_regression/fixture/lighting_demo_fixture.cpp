...
atlantis::Result<LightingDemoFixture, LightingDemoSetupError> setUpLightingDemoFixture(
  ...
  if (shadowCastVertexReflectionResult.isErr()) return ResultT::Err(LightingDemoSetupError::ShaderLoadFailed);
  const auto shadowCastVertexInputLayout = shadowCastVertexLayout(shadowCastVertexReflectionResult.value());
  if (!shadowCastVertexInputLayout.has_value()) return ResultT::Err(LightingDemoSetupError::ShaderLoadFailed);
  ...
  // (renderLightingDemoFrame()) stays empty; this fixture never
  // configures a real occluder.
  auto shadowMapResult = fixture.device->createShadowMap({.extent = {1024, 1024}});
  if (shadowMapResult.isErr()) return ResultT::Err(LightingDemoSetupError::ResourceCreationFailed);
  ...
  if (shadowMapSamplerResult.isErr()) return ResultT::Err(LightingDemoSetupError::ResourceCreationFailed);
  ...
  if (shadowCastPipelineResult.isErr()) return ResultT::Err(LightingDemoSetupError::ResourceCreationFailed);
  ...
  if (shadowLightSpaceBufferResult.isErr()) return ResultT::Err(LightingDemoSetupError::ResourceCreationFailed);
